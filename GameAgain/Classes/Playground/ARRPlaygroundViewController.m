...
- (void)initiateCountdownTime:(int)seconds
    ...
    [MusicPlayManager playMusic:@"dao.wav"];
    
    // Showing a countdown timer.
    [self showCountdownTime:5 withCompletionBlock:^{
        block();
    }];
}
...
- (void)showCountdownTime:(int)countdown
    ...
    UIColor *co1 = [UIColor redColor];
    UIColor *co2 = [UIColor orangeColor];
    UIColor *co3  = [UIColor blueColor];
    UIColor *co4 = [UIColor purpleColor];
    UIColor *co5 = [UIColor greenColor];
    UIColor *co6 = [UIColor blueColor];

    NSArray *CList = @[co1,co2,co3,co4,co5,co6,co1];
...
//    countLabel.textColor = [ARRTheme colorForArrowType:(countdown+1)];
    countLabel.textColor = CList[countdown];
    NSLog(@"=====%ld",countdown - 1);
